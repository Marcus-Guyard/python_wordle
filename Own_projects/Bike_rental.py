...
class BikeRental:
    ...
    def rent_bike_hourly(self, rent):
        if rent <= 0:
            print("Invalid input.")
            return None
        elif rent > self.stock:
            print(f"Sorry we only have {self.stock} amount of bikes for rent.")
            return None
        else:
            now = datetime.datetime.now()
            print("You have rented a {} bike(s) on hourly basis today at {} hours.".format(rent, now.hour))
            print("You will be charged $5 for each hour per bike.")
            print("We hope that you enjoy our service.")
    def rent_bike_daily(self):
        pass

    def rent_bike_weekly(self):
"""
import datetime
